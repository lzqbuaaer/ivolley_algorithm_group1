...
from ..feeder import myfeeder
from ..feeder import feeder
# torchlight

import src.G2.classify.torchlight
from src.G2.classify.torchlight.torchlight.io import str2bool
from src.G2.classify.torchlight.torchlight.io import DictAction
from src.G2.classify.torchlight.torchlight.io import import_class
...
class Processor(IO):
    ...
    def load_predict_data(self, datas):
        # Feeder = myfeeder.Feeder
        # self.data_loader['predict'] = torch.utils.pre_datas.DataLoader(
        #     dataset=Feeder(datas),
        #     batch_size=self.arg.test_batch_size,
        #     shuffle=False,
        #     num_workers=self.arg.num_worker * classify.torchlight.torchlight.ngpu(
        #         self.arg.device))
        self.datas = datas
    ...
    def load_data(self, T):
        ...
        if T == 'train':
            ...
            print(self.arg.num_worker * classify.torchlight.torchlight.ngpu(
                self.arg.device))
            self.data_loader['train'] = torch.utils.data.DataLoader(
                dataset=Feeder(**self.arg.train_feeder_args),
                batch_size=self.arg.batch_size,
                shuffle=True,
                num_workers=0,  # self.arg.num_worker * classify.torchlight.torchlight.ngpu(
                # self.arg.device),
                drop_last=False)
    ...
    def get_parser(add_help=False):
        ...
        # feeder
        parser.add_argument('--feeder', default='feeder.feeder', help='pre_datas loader will be used')
        parser.add_argument('--num_worker', type=int, default=4, help='the number of worker per gpu for pre_datas loader')
        parser.add_argument('--train_feeder_args', action=DictAction, default=dict(),
                            help='the arguments of pre_datas loader for training')
        parser.add_argument('--test_feeder_args', action=DictAction, default=dict(),
                            help='the arguments of pre_datas loader for test')
        parser.add_argument('--batch_size', type=int, default=256, help='training batch size')
        parser.add_argument('--test_batch_size', type=int, default=256, help='test batch size')
        parser.add_argument('--debug', action="store_true", help='less pre_datas, faster loading')
