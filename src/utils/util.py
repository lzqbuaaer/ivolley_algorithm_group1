...
from settings import AlgorithmPath
...
def point_dis_line(point, line_point1, line_point2):
    ...
    # 计算点到直线的距离
    distance_to_line = np.abs(np.cross(vec1, vec2)) / line_length
    # 计算点到线段端点的距离
    distance_to_point1 = np.linalg.norm(line_point1 - point)
    distance_to_point2 = np.linalg.norm(line_point2 - point)
    # 计算点到线段的方向向量
    line_direction = line_point2 - line_point1
    # 判断垂足是否在线段上
    dot_product1 = np.dot(vec1, line_direction)
    dot_product2 = np.dot(vec2, line_direction)
...
def arm_dis_ball(candidate, person, ball):
    try:
        ...
        p2 = num2pos(4, candidate, person)  # 小臂
    except:
    ...
    Log.error(f"ball is {ball}")
    ball_circle = [(ball[0] + ball[2]) / 2, (ball[1] + ball[3]) / 2]
    ball_radius = abs(ball[3] - ball[1]) / 2
    ball_to_arm = point_dis_line(ball_circle, p1, p2)  # 球中心到小臂
    Log.warning("球离手的距离是%f,球的半径是%f" % (ball_to_arm, ball_radius))
    Log.warning(f"比例为 ：{ball_to_arm / ball_radius}")
    return ball_to_arm / ball_radius


def draw_wrong_place(img, x, y):
    img_height, img_width, _ = img.shape
    circle_radius = int(min(img_height, img_width) * 0.05)
    cv2.circle(img, (int(x), int(y)), circle_radius, (0, 0, 255), int(circle_radius / 8))
...
def draw_messages(image, messages):
    ...
    # 设置字体参数
    font_path = os.path.join(AlgorithmPath, 'model', 'font', 'simhei.ttf')
    font_size = 50
...
def transfer(model, model_weights):  # 问一下这里什么意思
...
def draw_bodypose(canvas, candidate, subset):
    stickwidth = 1
    limbSeq = [[3, 6], [12, 6], [3, 4], [4, 5], [6, 7], [7, 8], [12, 9], [9, 10], \
               [10, 11], [2, 12], [12, 13], [13, 14], [3, 9]]
